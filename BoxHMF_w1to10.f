	...
	integer, parameter :: nh=20,tran=40000,iter=50000
	double precision   :: xx(nn),yy(nn),dy(nn)
	double precision   :: tau,dtau,r,phi,grnd
	double precision   :: KK,a,w,box
	integer            :: i,j,k,l,m,n,tran1,itlen,ll
	...
	open (unit=1,file='BoxHMF_w20.dat')
	...
	do n = 1, 30	! initial energy

	U0=n*0.05d0; H0=U0*mm
	...
	!write(1,'(100(xf12.6))') H/mm, 2*KE/mm, r
	...
	! function for box function

	function box(w,t)

	double precision :: w,t,TP,twopi,box
	integer          :: i,j,k,l,m,n

	twopi=8*datan(1.0d0)
	TP=twopi/w

	if (mod(t,TP)<TP/2) then
	box=1.0d0
	else
	box=0.0d0
	end if
