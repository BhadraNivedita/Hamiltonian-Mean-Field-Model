! integrates the driven hmf model by rk4 method
	...
	integer, parameter :: nh=10,tran=1,iter=10000
	double precision   :: xx(nn),yy(nn),dy(nn)
	double precision   :: tau,dtau,r,phi,grnd,r0
	double precision   :: KK,a,w,yyavg(nn)
	...
	U0=0.4d0; H0=U0*mm
	Uavg=0.0d0; Tavg=0.0d0; Ravg=0.0d0;R0avg=0.0d0
	yyavg=0.0d0
	...
	do i = 1, mm
	!yy(i) = (grnd()-0.5)*3.3	!r=0.6
	!yy(i) = (grnd()-0.5)*(.250d0+0.25d0*n)
	yy(i) = (grnd()-0.5)*0.5
	end do
	...
	!Tavg = Tavg/(nh*itlen)
	!Ravg = Ravg/(nh*itlen)
	!yyavg = yyavg/(nh*itlen)
	!R0avg = R0avg/(nh*itlen)
      	

	!do i=mm+1,nn
	!write(1,'(100(xf12.6))') yyavg(i)
	!end do
	
	!write(1,'(100(xf12.6))') Uavg,U0,R0avg, Ravg

             !write(*,*)  n,Uavg,U0,R0avg, Ravg   
